...
public class ContactsMemberTeacher {
	...
	public ContactsMemberTeacher(JSONObject jo){
		number = jo.optString("缂栧彿");               
		loginName = jo.optString("鐢ㄦ埛鍚?");          
		name = jo.optString("濮撳悕");                 
		nickname = jo.optString("鍛㈢О");             
		department = jo.optString("閮ㄩ棬");           
		password = jo.optString("瀵嗙爜");             
		orderNum = jo.optString("鎺掑簭鍙?");           
		sex = jo.optString("鎬у埆");                  
		birthday = jo.optString("鍑虹敓鏃ユ湡");         
		phone = jo.optString("鎵嬫満");                
		email = jo.optString("鐢甸偖");                
		mainRole = jo.optString("涓昏瑙掕壊");         
		aidRole = jo.optString("杈呭姪瑙掕壊");          
		prohibitLogin = jo.optString("绂佹鐧诲綍");    
		userNumber = jo.optString("鐢ㄦ埛鍞竴鐮?");
		virtualClass = jo.optString("铏氭嫙鐝骇");
		XingMing= jo.optString("XingMing");
		chargeClass= jo.optString("鎵?甯︾彮绾?");
		chargeKeCheng= jo.optString("鎵?甯﹁绋?");
	}
